    ...
    <div className={className} style={{ display: "grid", gridTemplateColumns: `repeat(${columns}, 1fr)`, gap: "8px 24px", ...style }}>
      {terms.map((t, i) => (
        <div key={i} style={{ display: "flex", gap: 8, alignItems: "stretch" }}>
          <span style={{ flex: "0 0 96px", background: "var(--deal-header-fill)", color: "var(--deal-header-text)", fontWeight: "var(--fw-bold)", fontSize: "var(--fs-body-sm)", padding: "4px 8px", borderRadius: 2, display: "flex", alignItems: "center", justifyContent: "center", textAlign: "center" }}>{t.term}</span>
